...
export default function SwapComponent({allList, allCurrency}){
    ...
    return(
        <div className="mx-auto flex justify-center items-center flex-col md:flex-row w-full w-max-full pt-5 px-5 container heighSwapComponenet"> 
            ...
            <span className='md:cursor-pointer py-4 px-4 bg-fondOne text-white w-10/12 mx-auto md:w-full mb-3 border-2 border-purple-400 rounded-2xl relative md:col-start-7 md:col-end-9' onClick={() => setSelectPool(!selectPool)}>
                {pool.poolName}
                <PoolImage pool={pool} />
            </span>
            <SwapBox selected={pool.assets}/>
                
                {/* Popup de pools posibles */}
            <ListOfPool poolData={list} selectPool={selectPool} handlreShow={selectNewPool}/>
            </div>
        </div>
    )
}
